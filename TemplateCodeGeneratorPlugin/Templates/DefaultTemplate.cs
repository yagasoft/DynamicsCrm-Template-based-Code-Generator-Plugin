...
#>
		...
		public System.Linq.IQueryable<<#= crmEntityName #>> <#= crmEntityName #>Set
		{
			get
			{
				return this.CreateQuery<<#= crmEntityName #>>().LimitAttributes();
			}
		}
<#}#>
	}
	...
	public static partial class CrmGeneratorExtensions
	{
			...
			where TEntity : GeneratedEntityBase
	    {
		    ...
		    return entities.GroupBy(e => e.Id).Select(ConsolidateEntity);
	    }
			...
			ref int depth, string intersectingEntity = null)
	    {
		    foreach (var xLink in xmlElement.Elements(""link-entity""))
		    {
			    ...
			    var relationProperty = targetEntity.GetType().GetProperties()
				    .Where(p => Attribute.IsDefined(p, typeof(RelationshipSchemaNameAttribute)))
				    .FirstOrDefault(
					    p =>
						{
							var a = p.GetCustomAttribute<RelationshipSchemaNameAttribute>();
							return a.SchemaName == schemaName && (a.PrimaryEntityRole == role || a.PrimaryEntityRole == null);
						});
			    ...
			    {
				    ProcessAttribute(baseEntity, relatedEntity, alias, xAttribute);
			    }
	    ...
	    private static void ProcessAttribute(Entity baseEntity, GeneratedEntityBase relatedEntity, string relatedAlias,
			XElement xAttribute)
	    {
		    var attributeAlias = xAttribute.Attribute(""alias"")?.Value ?? xAttribute.Attribute(""name"")?.Value;
		    ...
		    var attribute = baseEntity.Attributes
			    .FirstOrDefault(a => a.Key == $""{relatedAlias}.{attributeAlias}"").Value
			    ?? baseEntity.Attributes
				    .FirstOrDefault(a => a.Key == attributeAlias).Value;
		    ...
		    if (attribute != null)
		    {
			    relatedEntity[attributeAlias] = attribute;
		    }
	    }
		    ...
		    where TEntity : GeneratedEntityBase
	    {
		    ...
		    var baseRelationProperties = baseEntity.GetType().GetProperties()
			    .Where(p => Attribute.IsDefined(p, typeof(RelationshipSchemaNameAttribute)))
			    .Select(
				    p =>
					    new
						{
							a = p.GetCustomAttribute<RelationshipSchemaNameAttribute>(),
							p
						})
			    .Where(x => x.p.PropertyType.IsArray && (x.a.PrimaryEntityRole == EntityRole.Referenced
				    || x.a.PrimaryEntityRole == null)).ToArray();

		    foreach (var entity in grouping.Skip(1))
		    {
			    var relationProperties = entity.GetType().GetProperties()
				    .Where(p => Attribute.IsDefined(p, typeof(RelationshipSchemaNameAttribute)))
				    .Select(
					    p =>
						    new
				    ...
				    .Where(x => x.p.PropertyType.IsArray && (x.a.PrimaryEntityRole == EntityRole.Referenced
					    || x.a.PrimaryEntityRole == null));

			    foreach (var relationProperty in relationProperties)
			    {
				    var currentBaseRelationInfo = baseRelationProperties
					    .FirstOrDefault(x => x.a.SchemaName == relationProperty.a.SchemaName)?.p;
				    var currentRelation = (object[])relationProperty.p.GetValue(entity);
				    ...
				    Array.Copy(currentRelation, 0, newBaseRelation, baseLength, 1);

				    currentBaseRelationInfo.SetValue(baseEntity, newBaseRelation);
			    }
		    }

		    foreach (var baseRelationProperty in baseRelationProperties)
		    {
			    var currentRelation = (GeneratedEntityBase[])baseRelationProperty.p.GetValue(baseEntity);

			    if (currentRelation == null || currentRelation.Length <= 0)
			    {
				    continue;
			    }

			    var related = currentRelation
					.GroupBy(e => e.Id).Select(ConsolidateEntity).ToArray();
			    ...
			    baseRelationProperty.p.SetValue(baseEntity, newRelation);
		    }
...
<# 
	...
	foreach (var entity in selectedEntities)
    {
        ...
        foreach (var entityFilterT in entityProfiles.EntityProfilesHeaders
            .Where(entityFilterT => !entityFilterT.EntityProfiles.First(filter => filter.LogicalName == entity.LogicalName).IsExcluded)) {
...
<#+
	void Generate(MappingEntity entity, Context Context)
	{
		...
		var isContract = entityProfiles.EntityProfilesHeaders.Any(e => e.EntityProfiles.Any(filter => filter.LogicalName == entity.LogicalName && filter.IsExcluded == false));
        var entityFilter = Context.CrmEntityProfiles.FirstOrDefault(e => e.LogicalName == entity.LogicalName);
        var crmEntityName = (entityFilter == null || entityFilter.EntityRename == null)
			? (Context.UseDisplayNames ? entity.FriendlyName : entity.HybridName) : entityFilter.EntityRename;
		var fields = entity.Fields.Where(fieldQ => !fieldQ.Attribute.IsEntityReferenceHelper && fieldQ.IsValidForRead && (entityFilter == null || !isApplyEntityFilter || entity.PrimaryKey.LogicalName == fieldQ.Attribute.LogicalName || entityFilter.Attributes.Contains(fieldQ.Attribute.LogicalName)));
#>
...
<#+	foreach(var attribute in fields)	{
			var isAttributeContract = isContract && entityProfiles.EntityProfilesHeaders.Any(e => e.EntityProfiles.Any(filter => filter.LogicalName == entity.LogicalName && filter.IsExcluded == false && filter.Attributes != null && filter.Attributes.Contains(attribute.Attribute.LogicalName)));
            var attributeName = Context.UseDisplayNames ? (FirstNotNullOrEmpty((entityFilter == null || entityFilter.AttributeRenames == null) ? null : entityFilter.AttributeRenames, attribute.Attribute.LogicalName) ?? attribute.FriendlyName) : attribute.DisplayName;
...
<#+	
        ...
        foreach(var attribute in oneNRels.Where(e => entityFilter == null || !isApplyEntityFilter || entityFilter.OneToN.Contains(e.SchemaName)))	{
			var isAttributeContract = entityProfiles.EntityProfilesHeaders.Any(e => e.EntityProfiles.Any(filter => filter.LogicalName == entity.LogicalName && filter.IsExcluded == false && filter.OneToN != null && filter.OneToN.Contains(attribute.SchemaName)));
...
//BEGIN// N:1 Realationships 
	foreach(var attribute in nOneRels.Where(e => entityFilter == null || !isApplyEntityFilter || entityFilter.NToOne.Contains(e.SchemaName)))	{
			var isAttributeContract = entityProfiles.EntityProfilesHeaders.Any(e => e.EntityProfiles.Any(filter => filter.LogicalName == entity.LogicalName && filter.IsExcluded == false && filter.NToOne != null && filter.NToOne.Contains(attribute.SchemaName)));
			var profileTemp = Context.CrmEntityProfiles.FirstOrDefault(e => e.LogicalName == attribute.ToEntity.LogicalName);
			var crmEntityNameTemp = (profileTemp == null || profileTemp.EntityRename == null)
				? (Context.UseDisplayNames ? attribute.ToEntity.FriendlyName : attribute.ToEntity.HybridName) : profileTemp.EntityRename;
...
//BEGIN// N:N Relationships 
	foreach(var attribute in nNRels.Where(e => entityFilter == null || !isApplyEntityFilter || entityFilter.NToN.Contains(e.SchemaName)))	{
			var isAttributeContract = entityProfiles.EntityProfilesHeaders.Any(e => e.EntityProfiles.Any(filter => filter.LogicalName == entity.LogicalName && filter.IsExcluded == false && filter.NToN != null && filter.NToN.Contains(attribute.SchemaName)));
			var profileTemp = Context.CrmEntityProfiles.FirstOrDefault(e => e.LogicalName == attribute.ToEntity.LogicalName);
			var crmEntityNameTemp = (profileTemp == null || profileTemp.EntityRename == null)
				? (Context.UseDisplayNames ? attribute.ToEntity.FriendlyName : attribute.ToEntity.HybridName) : profileTemp.EntityRename;
...
<#+ //BEGIN// 1:N Realationships
	foreach(var attribute in oneNRels.Where(e => entityFilter == null || entityFilter.OneToN.Contains(e.SchemaName)))	{#>
<#+ if(attribute.SchemaName == ""calendar_calendar_rules"" || attribute.SchemaName == ""service_calendar_rules"") { #>
...
<#+ //BEGIN// N:1 Relationships 
	foreach(var attribute in nOneRels.Where(e => entityFilter == null || entityFilter.NToOne.Contains(e.SchemaName)))	{
			var relationName = FirstNotNullOrEmpty((entityFilter == null || entityFilter.NToOneRenames == null) ? null : entityFilter.NToOneRenames, attribute.SchemaName) ?? attribute.FriendlyName;
#>
				public const string <#= relationName #> = ""<#=attribute.SchemaName#>"";
<#+ } #>
...
<#+ //BEGIN// N:N Realationships 
	foreach(var attribute in nNRels.Where(e => entityFilter == null || entityFilter.NToN.Contains(e.SchemaName)).GroupBy(element => element.SchemaName).Select(group => group.First()))	{
			var relationName = FirstNotNullOrEmpty((entityFilter == null || entityFilter.NToNRenames == null) ? null : entityFilter.NToNRenames, attribute.SchemaName) ?? attribute.FriendlyName;
#>
				public const string <#= relationName #> = ""<#= attribute.SchemaName #>"";
<#+ } #>
...
<#+
	void GenerateContracts(EntityProfilesHeader header, Context Context)
	{
#>
...
<#+
		...
		foreach (var entityFilterQ in entityFilterT.EntityProfiles) {
			var entity = Context.Entities.FirstOrDefault(e => e.LogicalName == entityFilterQ.LogicalName
				&& Context.SelectedEntities.Contains(entityFilterQ.LogicalName));
			...
			if (entityFilterQ == null || entityFilterQ.IsExcluded == true)
			{
				continue;
			}
...
<#+
		foreach(var attribute in attributeR.ToEntity.Fields.Where(fieldQ => (!fieldQ.Attribute.IsEntityReferenceHelper && fieldQ.IsValidForRead && entityFilterQF.Attributes.Contains(fieldQ.Attribute.LogicalName)) || fieldQ.Attribute.LogicalName == attributeR.ToEntity.PrimaryKey.LogicalName))	{
            ...
            var attributeTypeName = Context.UseDisplayNames ? (FirstNotNullOrEmpty(entityFilterQF.AttributeRenames, attribute.Attribute.LogicalName) ?? attribute.FriendlyName) : attribute.DisplayName;
...
<#+ foreach(var enm in entity.Enums.Where(e => entityFilterQ.Attributes.Contains(e.LogicalName))) {
		var attribute = entity.Fields.FirstOrDefault(field => field.LogicalName == enm.LogicalName);
        var attributeName = Context.UseDisplayNames ? (FirstNotNullOrEmpty(entityFilterQ.AttributeRenames, attribute.Attribute.LogicalName) ?? enm.FriendlyName) : enm.DisplayName;
 #>
		[DataContract]
		public enum <#= attributeName #>Enum
		{
...
<#+
		...
		foreach (var entityFilterQ in entityFilterT.EntityProfiles) {
			...
			if (entityFilterQ == null || !entityFilterQ.IsGenerateMeta)
			{
				continue;
			}
...
<#+
	...
	void GenerateContracts()
	{
		...
		{
			GenerateContracts(entityFilter, Context);
		}
	}
#>
...
<#+
	void GenerateBase(Context Context)
	{
#>
