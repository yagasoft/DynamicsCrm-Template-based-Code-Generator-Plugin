...
<#@ assembly name=""Yagasoft.CrmCodeGenerator""#>
<#@ import namespace=""System.Linq"" #>
<#@ import namespace=""System.Collections.Generic"" #>
<#@ import namespace=""Yagasoft.CrmCodeGenerator.Models"" #>
<#@ import namespace=""Yagasoft.CrmCodeGenerator.Models.Mapper"" #>
...
//         Project / File: <#= Context.Namespace ?? ""<namespace>"" #> / <#= Context.FileName ?? ""<filename>"" #>.cs
...
<# } #>
	...
	public class LookupValue
	{
		...
		{
			get => (LookupEntity)Enum.Parse(typeof(LookupEntity), Reference.LogicalName, true);
			set => Reference.LogicalName = value.ToString().ToLower();
		}

		public string EntityName
		{
			get => Reference.LogicalName;
			set => Reference.LogicalName = value;
		}
		...
		public EntityReference Reference { get; } = new EntityReference();

		public LookupValue()
		{ }

		public LookupValue(EntityReference reference)
		{
			Reference = reference;
		}
	...
	[DebuggerNonUserCode]
	[DataContract]
	public class LookupKeys<TKey> : LookupValue, ILookupKeys<TKey> where TKey : LookupKeysBase
	{
		public LookupKeys(EntityReference reference) : base(reference)
		{ }

		public void AddKey(TKey key, object value)
		{
			Reference.KeyAttributes.Add(key.Name, value);
		}
	}
...
<#	var lookupKeyData = selectedEntities.SelectMany(e => e.Fields).Where(e => (e.LookupData == null ? null : (e.LookupData.LookupKeys == null ? null : e.LookupData.LookupKeys.Entity)) != null).Select(e => e.LookupData == null ? null : e.LookupData.LookupKeys)
...
<#+	foreach(var attribute in entity.Fields.Where(fieldQ => !fieldQ.Attribute.IsEntityReferenceHelper && fieldQ.IsValidForRead))	{
			...
			attributeType = (attribute.Attribute != null && attribute.Attribute.IsMultiTyped == true && Context.IsUseCustomEntityReference) ? ""LookupValue"" : attributeType;
			var crmType = attribute.IsStateCode ? ""OptionSetValue"" : (attribute.EnumData != null && attribute.EnumData.IsMultiSelect == true ? ""OptionSetValueCollection"" : attribute.TargetTypeForCrmSvcUtil);
#>
...
<#+ if(attribute.Description != """") { #>
        ...
        /// </summary>
<#+ } #>
		[AttributeLogicalName(""<#= attribute.Attribute.LogicalName #>"")<#+ if (entityFilter == null || entityFilter.Attributes == null || entityFilter.Attributes.Contains(attribute.Attribute.LogicalName)) { #>, DataMember<#+ } #><#+ if (Context.IsAddEntityAnnotations) { #><#+ if (attribute.IsRequired) { #>, Required<#+ } #><#+ if (attribute.MaxLength != null) { #>, MaxLength(<#= attribute.MaxLength #>)<#+ } #><#+ if (attribute.MaxLength != null && attributeType == ""string"") { #>, StringLength(<#= attribute.MaxLength #>)<#+ } #><#+ if (attribute.Min != null || attribute.Max != null) { #>, Range(<#= attribute.Min #><#= attribute.Max == null ? """" : ("","" + attribute.Max) #>)<#+ } #><#+ if (attribute.ImageData != null && attribute.ImageData.MaxSizeInKb != null && attribute.ImageData.MaxWidth != null && attribute.ImageData.MaxHeight != null) { #>, MaxLength(<#= attribute.ImageData.MaxSizeInKb * 1000 #>), MaxWidth(<#= attribute.ImageData.MaxWidth #>), MaxHeight(<#= attribute.ImageData.MaxHeight #>)<#+ } #><#+ } #>]
...
<#+ } else if (attribute.TargetTypeForCrmSvcUtil.Contains(""Money"")) { #>
                return value?.Value;
<#+ } else if (attribute.Attribute != null && attribute.Attribute.IsMultiTyped == true && Context.IsUseCustomEntityReference) { #>
                return value == null ? null : new LookupValue(GetAttributeValue<EntityReference>(""<#= attribute.Attribute.LogicalName #>""));
...
<#+ else if (attribute.TargetTypeForCrmSvcUtil.Contains(""Money"")) { #>
	...
	<#+ } #>
<#+ else if (attribute.Attribute != null && attribute.Attribute.IsMultiTyped == true && Context.IsUseCustomEntityReference) { #>
                if (value != null) SetAttributeValue(""<#= attribute.Attribute.LogicalName #>"", value.Reference);
                else
	<#+ } #>
<#+ else if (attribute.TargetTypeForCrmSvcUtil.Contains(""EntityReference"") && !attributeType.Contains(""EntityReference"")) { #>
                if (value != null) SetAttributeValue(""<#= attribute.Attribute.LogicalName #>"", new EntityReference(""<#= attribute.LookupData == null ? null : attribute.LookupData.LookupSingleType #>"", value.Value));
                else
	<#+ } #>
...
<#+ if (entityFilter == null || entityFilter.Attributes == null || entityFilter.Attributes.Contains(attribute.Attribute.LogicalName)) { #>
        [DataMember]
<#+ } #>
<#+ if (attribute.LookupData != null && attribute.LookupData.LookupLabel != null && !string.IsNullOrEmpty(attribute.LookupData == null ? null : attribute.LookupData.LookupLabel.LabelFieldNames)) { #>
		[Label(""<#= attribute.LookupData == null ? null : attribute.LookupData.LookupLabel.LabelFieldNames #>"", ""<#= attribute.LookupData == null ? null : attribute.LookupData.LookupLabel.LogicalName #>"", ""<#= attribute.LookupData == null ? null : attribute.LookupData.LookupLabel.IdFieldName #>"", ""<#=attribute.Attribute.LogicalName#>"")]
<#+ } #>
...
<#= relationProperties #>
		...
		/// </summary>
		public <#= crmEntityName #>(object anonymousType) : this()
		{
...
#>
						{ <#= grp.Key #>,
							...
							{
<#+ foreach(var item in enm.Items) { #>
								{<#= item.Value #>, <#= ToLiteral(item.LocalizedLabels.FirstOrDefault(e => e.LanguageCode == grp.Key).Label) #>},
<#+ } #>
							}
						}
<#+ } #>
...
<#+ if (f.LocalizedLabels != null) { #>
				public static IDictionary<int, string> <#= Context.UseDisplayNames ? (FirstNotNullOrEmpty(entityFilter.AttributeRenames, f.Attribute.LogicalName) ?? f.FriendlyName) : f.DisplayName #> { get; set; } =
					new Dictionary<int, string>
					{
<#+ foreach(var grp in f.LocalizedLabels.GroupBy(e => e.LanguageCode)) { #>
						{ <#= grp.Key #>, <#= ToLiteral(grp.FirstOrDefault(e => e.LanguageCode == grp.Key).Label) #>},
<#+ } #>
					};
...
<#+	foreach(var attribute in entity.Fields.Where(fieldQ => !fieldQ.Attribute.IsEntityReferenceHelper && fieldQ.IsValidForRead && entityFilterQ.Attributes.Contains(fieldQ.Attribute.LogicalName)))	{
			...
			var attributeType = ConvertToContractType(attribute, attributeName);
			attributeType = (attributeType.Contains(""Enum?"") || attributeType.Contains(""Enum[]"")) ? (crmEntityName + ""."" + attributeType) : attributeType;
			attributeType = (attribute.Attribute != null && attribute.Attribute.IsMultiTyped == true && Context.IsUseCustomEntityReference) ? ""LookupValue"" : attributeType;
#>
		[DataMember, CrmFieldMapping(""<#=attribute.Attribute.LogicalName#>""<#+ if (attribute.TargetTypeForCrmSvcUtil.Contains(""EntityReference"") && !attributeType.Contains(""EntityReference"") && (attribute.Attribute == null || !attribute.Attribute.IsMultiTyped)) { #>, ""<#= attribute.LookupData == null ? null : attribute.LookupData.LookupSingleType #>""<#+ } #>)<#+ if (Context.IsAddContractAnnotations) { #><#= (entityFilterQ.ReadOnly != null && entityFilterQ.ReadOnly.Contains(attribute.LogicalName)) ? "", ReadOnlyField"" : """" #><#+ if (attribute.IsRequired) { #>, Required<#+ } #><#+ if (attribute.MaxLength != null) { #>, MaxLength(<#= attribute.MaxLength #>)<#+ } #><#+ if (attribute.MaxLength != null && attributeType == ""string"") { #>, StringLength(<#= attribute.MaxLength #>)<#+ } #><#+ if (attribute.Min != null && attribute.Max != null) { #>, Range(<#= attribute.Min #><#= attribute.Max == null ? """" : ("","" + attribute.Max) #>)<#+ } #><#+ if (attribute.ImageData != null && attribute.ImageData.MaxWidth != null && attribute.ImageData.MaxHeight != null) { #>, MaxLength(<#= attribute.ImageData.MaxSizeInKb * 1000 #>), MaxWidth(<#= attribute.ImageData.MaxWidth #>), MaxHeight(<#= attribute.ImageData.MaxHeight #>)<#+ } #><#+ } #>] public <#= attributeType #> <#= attributeName #> { get; set; }
...
<#+ if (entityFilterQ.IsLookupLabels) { #>
        [DataMember<#+ if ((attribute.LookupData == null ? null : attribute.LookupData.LookupLabel) != null && !string.IsNullOrEmpty(attribute.LookupData == null ? null : attribute.LookupData.LookupLabel.LabelFieldNames)) { #>, Label(""<#= attribute.LookupData == null ? null : attribute.LookupData.LookupLabel.LabelFieldNames #>"", ""<#= attribute.LookupData == null ? null : attribute.LookupData.LookupLabel.LogicalName #>"", ""<#= attribute.LookupData == null ? null : attribute.LookupData.LookupLabel.IdFieldName #>"", ""<#=attribute.Attribute.LogicalName#>"")<#+ } #>] public <#= Context.IsUseCustomDictionary ? ""List<OptionsetLanguageLabel>"" : ""IDictionary<int, string>"" #> <#= attributeName #>Labels { get; set; }
<#+ } #>
<#+ } #>
		...
		public <#= Context.IsUseCustomDictionary ? ""List<OptionsetLanguageLabel>"" : ""IDictionary<int, string>"" #> <#= attributeName #>Labels
		{
			get
			{
				return <#= attributeName #> == null
					? null
					: <#= Context.IsUseCustomDictionary ? ""new List<OptionsetLanguageLabel>"" : ""new Dictionary<int, string>"" #>
							{
<#+ foreach(var language in languages) { #>
								<#= Context.IsUseCustomDictionary ? ""new OptionsetLanguageLabel"" : """" #> { <#= Context.IsUseCustomDictionary ? ""LanguageCode = "" : """" #><#= language #>, <#= Context.IsUseCustomDictionary ? ""OptionsetLabel = "" : """" #><#= crmEntityName #>.Enums.GetLabel(""<#=attribute.Attribute.LogicalName#>"", (int) <#= attributeName #>, <#= language #>) },
<#+ } #>
							};
			}
			set {}
...
<#+ } else { #>
		[DataMember, CrmRelationMapping(""<#=attribute.SchemaName#>"", typeof(<#= crmEntityNameTemp #>)<#=attribute.EntityRole == ""null"" ? """" :"", "" + attribute.EntityRole#>)<#+ if (Context.IsAddContractAnnotations) { #><#= isReadOnly ? "", ReadOnlyField"" : """" #><#+ } #>] public <#= prefix + contractEntityNameTemp + suffix #>[] <#= relationName #> { get; set; }
<#+ } #>
<#+ } //END// 1:N Realationships #>
...
#>
		[<#= flatten ? """" : ""DataMember, "" #>CrmRelationMapping(""<#=attributeR.SchemaName#>"", typeof(<#= crmEntityNameTemp #>)<#=attributeR.EntityRole == ""null"" ? """" :"", "" + attributeR.EntityRole#>)<#+ if (Context.IsAddContractAnnotations) { #><#= isReadOnly ? "", ReadOnlyField"" : """" #><#+ } #>] public <#= prefix + contractEntityNameTemp + suffix #> <#= relationName #> { get; set; }
...
<#+
		foreach(var attribute in attributeR.ToEntity.Fields.Where(fieldQ => (!fieldQ.Attribute.IsEntityReferenceHelper && fieldQ.IsValidForRead && entityFilterQF.Attributes.Contains(fieldQ.Attribute.LogicalName)) || fieldQ.Attribute.LogicalName == attributeR.ToEntity.PrimaryKey.LogicalName))	{
				...
				var attributeType = ConvertToContractType(attribute, attributeName);
				attributeType = (attributeType.Contains(""Enum?"") || attributeType.Contains(""Enum[]"")) ? (crmEntityNameTemp + ""."" + attributeType) : attributeType;
				attributeType = (attribute.Attribute != null && attribute.Attribute.IsMultiTyped == true && Context.IsUseCustomEntityReference) ? ""LookupValue"" : attributeType;
#>
		[DataMember, CrmFieldMapping(""<#=attribute.Attribute.LogicalName#>""<#+ if (attribute.TargetTypeForCrmSvcUtil.Contains(""EntityReference"") && !attributeType.Contains(""EntityReference"")) { #>, ""<#= attribute.LookupData == null ? null : attribute.LookupData.LookupSingleType #>""<#+ } #>), CrmRelationMapping(""<#=attributeR.SchemaName#>"", typeof(<#= crmEntityNameTemp #>)<#=attributeR.EntityRole == ""null"" ? """" :"", "" + attributeR.EntityRole#>)<#+ if (Context.IsAddContractAnnotations) { #><#= (entityFilterQ.ReadOnly != null && entityFilterQ.ReadOnly.Contains(attribute.LogicalName)) || isReadOnly ? "", ReadOnlyField"" : """" #><#+ } #>] public <#= attributeType #> <#= relationName #>_<#= attributeName #> { get; set; }
<#+ if (attribute.TargetTypeForCrmSvcUtil.Contains(""EntityReference"")) { #>
        [DataMember] public string <#= relationName #>_<#= attributeName #>Name { get; set; }
<#+ if (entityFilterQF.IsLookupLabels) { #>
        [DataMember<#+ if ((attribute.LookupData == null ? null : attribute.LookupData.LookupLabel) != null && !string.IsNullOrEmpty(attribute.LookupData == null ? null : attribute.LookupData.LookupLabel.LabelFieldNames)) { #>, Label(""<#= attribute.LookupData == null ? null : attribute.LookupData.LookupLabel.LabelFieldNames #>"", ""<#= attribute.LookupData == null ? null : attribute.LookupData.LookupLabel.LogicalName #>"", ""<#= attribute.LookupData == null ? null : attribute.LookupData.LookupLabel.IdFieldName #>"", ""<#=attribute.Attribute.LogicalName#>"")<#+ } #>] public <#= Context.IsUseCustomDictionary ? ""List<OptionsetLanguageLabel>"" : ""IDictionary<int, string>"" #> <#= relationName #>_<#= attributeName #>Labels { get; set; }
<#+ } #>
<#+ } #>
...
#>
        ...
        public <#= Context.IsUseCustomDictionary ? ""List<OptionsetLanguageLabel>"" : ""IDictionary<int, string>"" #> <#= relationName #>_<#= attributeName #>Labels
		{
			get
			{
				return <#= relationName #>_<#= attributeName #> == null
            		? null
					: <#= Context.IsUseCustomDictionary ? ""new List<OptionsetLanguageLabel>"" : ""new Dictionary<int, string>"" #>
							{
<#+ foreach(var language in languages) { #>
...
<#+
	...
	private static string ConvertToContractType(MappingField field, string attributeName)
	{
        ...
        else if (field.EnumData != null && field.EnumData.IsMultiSelect == true)
        {
            return attributeName + ""Enum[]"";
        }
